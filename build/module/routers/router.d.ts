...
export declare abstract class IRouter<RoutingConfig> {
     ...
     */
    abstract route(amount: CurrencyAmount, quoteCurrency: Currency, swapType: TradeType, factoryAddress: string, initCodeHash: string, swapOptions?: SwapOptions, partialRoutingConfig?: Partial<RoutingConfig>): Promise<SwapRoute | null>;
}
export declare abstract class ISwapToRatio<RoutingConfig, SwapAndAddConfig> {
    abstract routeToRatio(token0Balance: CurrencyAmount, token1Balance: CurrencyAmount, position: Position, factoryAddress: string, initCodeHash: string, swapAndAddConfig: SwapAndAddConfig, swapAndAddOptions?: SwapAndAddOptions, routingConfig?: RoutingConfig): Promise<SwapToRatioResponse>;
}
